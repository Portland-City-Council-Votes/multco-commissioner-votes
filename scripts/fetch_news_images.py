...
def og_image(url, headline):
    out = subprocess.run(["curl", "-sSL", "--max-time", "30", "-A", "Mozilla/5.0 (multco-commissioner-votes)", url],
                         capture_output=True)
    if out.returncode:
        raise OSError(out.stderr.decode("utf-8", "replace").strip() or f"curl exit {out.returncode}")
    page = out.stdout[:600_000].decode("utf-8", "replace")
